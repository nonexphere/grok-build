...
export interface InitializeParams { protocolVersion: "2026-07-18.experimental-v2"; clientInfo: ClientInfo; capabilities: ClientCapabilities }
...
export interface ProviderBinding { providerId: string; credentialId: string; modelId: string; backend: string; bindingRevision: WireCounter }
export type SessionStatus = "starting" | "ready" | "running" | "waiting_for_input" | "dormant" | "completed" | "failed" | "archived";
export interface Session { sessionId: string; historyEpoch: string; revision: WireCounter; status: SessionStatus; workspaceRoot: string; title: string | null; activeTurnId: string | null; latestTurnId: string | null; providerBinding: ProviderBinding | null; createdAtMs: number; updatedAtMs: number }
export type TurnStatus = "queued" | "in_progress" | "waiting_for_approval" | "waiting_for_input" | "completed" | "failed" | "interrupted" | "declined";
export interface Turn { turnId: string; sessionId: string; providerBinding: ProviderBinding | null; ordinal: number; kind: "user" | "steer" | "resume" | "synthetic"; status: TurnStatus; revision: WireCounter; createdAtMs: number; completedAtMs: number | null }
export interface ItemBase { itemId: string; sessionId: string; turnId: string; status: string; revision: WireCounter; eventSeq: WireCounter; createdAtMs: number }
...
export interface SessionStartParams { workspaceRoot: string; agentType?: string | null; providerBinding?: ProviderBinding | null; idempotencyKey: string }
...
export interface SubscribeParams { sessionId: string; historyEpoch?: string | null; afterEventSeq: WireCounter }
export interface SubscribeResult { subscriptionId: string; historyEpoch: string; replayedThroughEventSeq: WireCounter; liveFromEventSeq: WireCounter }
export interface OperationResult { operationId: string; accepted: boolean }
export interface ProtocolEvent { method: string; params: { subscriptionId?: string; sessionId?: string; historyEpoch?: string; eventSeq?: WireCounter; [key: string]: unknown } }
