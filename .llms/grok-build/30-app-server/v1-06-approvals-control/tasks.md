...
- [ ] `AS106-01` [D-AP.1] In App Server controller modules, implement UNOWNED/HELD/RELEASED/RESOLVED lease transitions; run `./scripts/run-rust-test-gate.sh controller_lease cargo test -p xai-grok-app-server controller_lease`; accept revisioned exactly-one controller behavior.
- [ ] `AS106-02` [D-AP.2,D-SP.13] Keep Interaction ID distinct from connection request ID; run `./scripts/run-rust-test-gate.sh interaction_identity cargo test -p xai-grok-app-server interaction_identity`; accept retry/reconnect preserves Interaction identity.
