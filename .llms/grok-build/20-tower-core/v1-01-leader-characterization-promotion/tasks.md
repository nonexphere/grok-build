...
- [x] `TW101-04` [D-TW.13,D-TW.14] Add registry ownership tests in `xai-grok-tower/tests/one_actor.rs` plus the shell adapter tests; run `./scripts/run-rust-test-gate.sh one_actor cargo test -p xai-grok-tower -p xai-grok-shell one_actor`; accept mutations serialized by the existing actor.
- [x] `TW101-05` [D-TD.3] Record RED/GREEN commands and observed failure in PR evidence; run `./scripts/run-rust-test-gate.sh leader_characterization cargo test -p xai-grok-tower leader_characterization`; accept reproducible test names.
