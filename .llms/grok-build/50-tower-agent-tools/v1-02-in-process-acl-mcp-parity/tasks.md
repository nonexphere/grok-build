...
- [x] `TA102-02` [D-TA.6] Register descriptors in-process through the runtime tool composition path; run `./scripts/run-rust-test-gate.sh in_process_registration cargo test -p xai-grok-tower-tools in_process_registration`; accept direct facade calls with no JSON-RPC/MCP loop.
