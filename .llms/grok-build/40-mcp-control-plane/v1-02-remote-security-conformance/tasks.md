...
- [x] `MCP102-03` [D-SEC.8,D-SEC.10] Enforce body/queue/SSE limits in MCP transports; run `./scripts/run-rust-test-gate.sh limits cargo test -p xai-grok-mcp-server limits`; accept explicit resync and no silent drop.
...
- [x] `MCP102-05` [D-MCP.7] Run shared fixtures against HTTP and stdio drivers under `tests/adapter_parity.rs`; run `./scripts/run-rust-test-gate.sh conformance cargo test -p xai-grok-mcp-server conformance`; accept identical tool semantics.
