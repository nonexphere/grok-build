...
| **A1 attempt stamp FIFO + peek** | **PASS** | `AttemptStampLedger` + `auth_info_and_prefix_use_peek_bearer_not_send` |
| **A2+R5 refresh single-flight** | **PASS (multi-manager / file flock)** | `two_managers_same_file_home_one_refresh_via_xproc_lock` |
...
| **PC8 live cache** | **PASS (live)** | `goblin -p` Codex `gpt-5.6-luna`: turn1 `cache_read_input_tokens=0`, turn2 `-c` continue **`17920`** (>0); evidence `/tmp/grok-goal-ed5090e87b2e/implementer/pc8-live.md` |

Prompt cache mental model: **backend owns cache**; client passes stable prefix + optional opaque affinity key + observes `cache_read_input_tokens` / `cached_tokens`. Live multi-turn hit proven via goblin binary + logged-in Codex account (2026-07-17).

**Production-ready claim scope:** single-machine multi-process offline Codex path (flock refresh, journal, wire, pin, catalog, capability gate, compaction key, title guard). **Not** claimed: live cache hits, multi-host refresh, multi-provider 1.0 (keyring/xAI/D10).
...
> Goblin includes a multi-provider Codex path: credential-scoped models, request-time OAuth bearer with attempt-bound stamps and cross-process refresh flock, OpenResponses phase/commentary + function-call wire siblings, capability/binding-aware Codex gate, opaque `prompt_cache_key` (incl. compaction), and per-credential model catalog cache.  
> **Live prompt-cache hit proof** remains gated (`GROK_LIVE_CODEX`). Login is fail-closed / opt-in (D10). See `TO_RELEASE.md`.
